...
## Phase 72.2 — Strategy Session Funnel Analytics (July 2026)
- Measures homepage visit → Strategy Session booking. Additive only — 72.1, auth, Concierge, Stripe, Calendly untouched.
- `STRATEGY_FUNNEL_EVENTS` (14-event closed enum) + `strategyFunnelEvents` table (nullable session_id/visitor_id, attribution cols page/referrer/source/medium/campaign/device/browser, metadata jsonb, 4 indexes) + `funnelAnalyticsRequestSchema` (`.strip()`, attr strings ≤300, page/referrer ≤600) in `shared/schema.ts`; response types in `shared/types/funnel.ts`.
- `POST /api/analytics/funnel` (public, rateLimit 60/min, 400 unknown event, 413 metadata >2048B serialized, `{"ok":true}`); `GET /api/dashboard/analytics/funnel` (requireDashboardAuth) — `getFunnelAnalyticsSummary` is pure SQL (per-stage `count(distinct coalesce(session_id,'anon-'||id)) FILTER`, visitors from `page_views WHERE page='/'`, avg completion via `min() FILTER` subquery, top sources/campaigns/plans GROUP BY LIMIT 10, daily 30d/weekly 12w/monthly 12m `date_trunc` — trunc unit inlined via `sql.raw` from closed union, parameterized $1 in SELECT+GROUP BY → PG 42803).
- Stages: visitors→strategy(strategy_page_view)→pricing(pricing_view|pricing_section_view)→plan(plan_selected, top plans from `metadata->>'plan'`)→checkout(checkout_started)→payment(checkout_completed)→booked(booking_completed).
...
- Wired events: strategy_page_view + checkout_started (StrategySession), pricing_view (Pricing), checkout_completed + thank_you_view (CheckoutSuccess, gated `source==="strategy-session"`). plan_selected/booking_* are API-ready but not yet emitted anywhere (booking flow is Calendly-side — do-not-touch).
...
- **Prod deploy step: run `npx tsx scripts/create_phase72_2_tables.ts` against Render DATABASE_URL — summary endpoint 500s until table exists.** Tests: `npx tsx scripts/phase72_2_api_tests.ts` (16 cases, all pass). check+build PASS; architect PASS.

## Phase 72.3 — Revenue Intelligence System (July 2026)
- Founder-only revenue analytics atop 72.1/72.2. Additive only. Route is **`/revenue-analytics`** (NOT `/revenue-intelligence` — Phase 65 owns that).
- `shared/schema.ts`: `REVENUE_SOURCES`(15) / `REVENUE_EVENT_TYPES`(21) / `CLIENT_REVENUE_EVENTS`(5 engagement-only) / `REVENUE_EARNING_EVENTS`(9) / `ATTRIBUTION_MODELS` closed enums; `revenue_intelligence_events` table (11 indexes + partial unique `revenue_intel_dedupe_key_uq` WHERE dedupe_key IS NOT NULL); `revenueAnalyticsRequestSchema` (`.strip()`, amount 0–50M cents, metadata ≤2048B → 413). Types in `shared/types/revenue.ts`.
- **Trust boundary** `POST /api/analytics/revenue` (public, rateLimit 60/min): anonymous callers limited to `CLIENT_REVENUE_EVENTS` (else 403); `amountCents` forced 0 and dedupeKey/occurredAt/orderId/Stripe ids/userId/leadId stripped — engagement events can never claim money or squat dedupe keys. Dashboard-authed callers (session or `x-dashboard-pin`) may submit economic events. `GET /api/dashboard/analytics/revenue` (requireDashboardAuth) — `getRevenueIntelSummary` pure SQL (KPIs, 9 top-10 breakdowns, daily/weekly/monthly trends, visitors→leads→bookings→paid economic funnel, diagnostics, attributionNote). Only `REVENUE_EARNING_EVENTS` count toward money KPIs.
- **Stripe webhook** (single existing endpoint): after order fulfillment records `payment_completed` (or `subscription_started`/membership when `session.mode==="subscription"` — Phase 68A signups excluded from paidOrderCount/AOV), auto dedupe key `stripe:{event}:{sessionId}` in storage (`onConflictDoNothing` on partial unique index → retries can't double-count). `charge.refunded` records per-refund rows keyed `stripe:refund:{refund.id}` (cumulative-once fallback keyed by charge.id when refunds list omitted — a 2nd partial refund under-counts in that fallback, never over-counts); `payment_intent.payment_failed` deduped by event id. Refund/failed handlers are inert until those events are enabled on the Stripe webhook endpoint in the dashboard.
- **CORRECTIVE 72.2 funnel normalization**: `getFunnelAnalyticsSummary` now also returns `normalizedStages`/`normalizedConversions` (per-session furthest stage via max(CASE), cumulative counts → monotonically non-increasing, conversions capped 0–100) + `diagnostics` (`outOfOrderSessions`, `duplicateEvents`); `overall` uses normalized counts. Raw `stages`/`conversions` keys preserved (additive). FunnelAnalytics UI: normalized/raw toggle (normalized default) + Data Integrity card.
...
- **Prod deploy step: run `npx tsx scripts/create_phase72_3_tables.ts` against Render DATABASE_URL — revenue summary 500s until table exists.** Also enable `charge.refunded` + `payment_intent.payment_failed` on the Stripe webhook endpoint (prod dashboard) to activate refund/failure tracking. Tests: `npx tsx scripts/phase72_3_api_tests.ts` (30 cases, all pass; needs running server + `DASHBOARD_PIN`). check+build PASS; architect PASS (2 medium webhook-accuracy findings fixed: subscription-mode guard, per-refund dedupe).

## Phase 72.4 — Search Intelligence & Authority Platform (July 2026)
- Founder-only first-party search & content-authority analytics atop 72.1–72.3. Additive only. Route **`/search-intelligence`** (PIN gate, concierge-hidden). No external APIs (no Search Console/Ahrefs) — referrer/UTM classification only; honest limits in `attributionNote` (AI referrers often stripped → ai_assistant undercounts; no keyword data; bots/no-JS invisible).
- `shared/schema.ts`: `SEARCH_TRAFFIC_SOURCES`(12 incl. `ai_assistant`) / `ORGANIC_SEARCH_SOURCES`(6) / `SEARCH_INTEL_EVENTS`(5: search_landing, content_view, content_read ≥50% scroll, content_complete ≥90%+≥30s dwell, content_share) / `SEARCH_CONTENT_TYPES`(blog|knowledge|page); `search_intelligence_events` table (7 indexes + partial unique `search_intel_dedupe_key_uq` WHERE dedupe_key IS NOT NULL); `searchIntelRequestSchema` (`.strip()` + superRefine: landing requires trafficSource, content events require contentSlug; readPercent 0–100, dwell ≤14400s, attrs ≤300/paths ≤600 chars, metadata >2048B → 413). Types in `shared/types/searchIntel.ts`.
- **Trust boundary** `POST /api/analytics/search` (public, rateLimit 60/min): `dedupeKey` is NOT in the request schema — storage derives `sil:landing:{sid}` / `sil:{event}:{sid}:{slug}` / `sil:share:{sid}:{slug}:{channel}` (skipped when session missing/`'anon'`), so clients can never squat idempotency keys; empty/whitespace sessionId/visitorId normalized to NULL at ingestion and summary SQL treats NULL/`'anon'`/`''` as anonymous (`''` can never masquerade as one shared "real" session); no money fields exist on this pipeline; Authority Index is SQL-computed only. `GET /api/dashboard/analytics/search` (requireDashboardAuth) — `getSearchIntelSummary` pure SQL: KPIs, full-12-source breakdown joined to 72.2 funnel + 72.3 revenue by session id (`attributedRevenueCents` sums REVENUE_EARNING_EVENTS only — a lower bound since Stripe webhook rows lack browser session ids), top-10 referrer hosts/landing paths/campaigns, content authority (top 20), daily/weekly/monthly trends, content footprint (published-never-viewed posts), integrity diagnostics.
- **Authority Index** (0–100 per slug, transparent, NOT third-party domain authority): `round(100·(0.25·reach + 0.35·depth + 0.25·completion + 0.15·amplification))`; reach/amplification log-scaled vs max slug, depth = avg max read % per viewing session, completion = completes/views. Per-session grouping via `CASE WHEN session_id IS NULL OR 'anon' THEN 'row-'||id` so storage-blocked visitors never merge into one fake session.
- Client: `client/src/lib/searchIntelligence.ts` — classifier (AI-assistant hosts chatgpt/perplexity/claude/gemini/copilot checked BEFORE the google regex; gclid/msclkid/utm_medium cpc|ppc|ads→paid; email/newsletter→email; `UTM_SOURCE_ALIASES` maps non-host utm_source values; same-origin/no referrer→direct); reuses 72.2 keys `e360_visitor_id`/`e360_funnel_session`/`e360_funnel_attribution` (identical capture logic) so cross-phase session joins work. `SearchLandingTracker` mounted in App.tsx next to RouteTracker (fires once per session — certified RouteTracker untouched); `useContentEngagement` hook (client/src/hooks/) wired in BlogPost (`blog/{slug}`) + ArticleDetail (`knowledge/{slug}`); ShareButton added to BlogPost footer with capture-phase `content_share` (ShareButton itself unmodified).
- Dashboard `client/src/pages/SearchIntelligence.tsx`: mirrors FunnelAnalytics (shared `e360_dashboard_auth` session flag + `/api/dashboard/auth` PIN post) — 8 KPI cards, source bars w/ funnel/revenue join chips, authority table, Search→Outcomes card, top-10 lists, footprint + data-integrity cards, daily/weekly/monthly trends, formula/attribution footer.
- **Prod deploy step: run `npx tsx scripts/create_phase72_4_tables.ts` against Render DATABASE_URL — search summary 500s until table exists.** Tests: `npx tsx scripts/phase72_4_api_tests.ts` (50 checks: happy paths, closed-vocab 400s, 413, dedupe idempotency via summary deltas, client-dedupeKey squat guard, empty-session anonymity, cross-phase session joins, anonymous-money integrity, SPA-fallback guard, 72.1/72.2/72.3 regression guards; needs running server + `DASHBOARD_PIN`). check+build PASS; architect round: 1 medium finding (empty-string sessionId stored as a joinable "real" session) fixed + regression-tested.

## Phase 72.4R — Search Intelligence Revision: GSC, SEO Audits, CWV, Organic Revenue (July 2026)
- Additive revision on 72.4 (all first-party work preserved verbatim in `FirstPartyOverview`). Adds Google Search Console integration, on-site SEO audits, Core Web Vitals RUM, organic revenue attribution, and a 12-tab dashboard refactor at `/search-intelligence` (overview, queries, pages, seo-health, structured, metadata, social, indexing, vitals, organic-revenue, recommendations, sync).
- **Env vars (both optional)**: `GOOGLE_SEARCH_CONSOLE_CREDENTIALS` (service-account JSON, property must grant the SA email access) + `GSC_SITE_URL` (e.g. `sc-domain:elevate360official.com`). Unset → every surface reports typed `not_configured` with remediation reason; nothing 500s, dashboard stays fully usable.
- **GSC client** `server/services/googleSearchConsole.ts`: hand-rolled JWT RS256 (no googleapis dep), paginated Search Analytics pulls (queries/pages/dimensions/query+page), dedupe+chunked upserts into 5 `gsc_*` tables. Dashboard GETs NEVER call Google — founder-triggered `POST /api/dashboard/search-intelligence/sync` only (PIN, rateLimit 6/300s, zod scope all|gsc|audits + days 1–90, one active GSC run at a time → 409, `fixture` import → 403 in production). Re-import idempotent (regression-tested: row counts + window totals + per-query metrics unchanged).
- **SEO audits** `server/services/seoAudit.ts`: self-fetches server-delivered HTML from `127.0.0.1:PORT` — audits title/meta/OG/Twitter/canonical/H1/JSON-LD/robots/sitemap per route. In dev the SPA serves one index.html → duplicate titles are EXPECTED honest findings (disclosed in `metadata.note`); prod (meta-image/SSR-tagged pages) audits real per-route HTML. Results → `seo_*` tables; recommendations rebuilt per view by pure `buildRecommendations()` (severity-sorted, cap 20, GSC-not-connected rule guarantees ≥1 in dev).
- **Core Web Vitals**: `client/src/lib/webVitals.ts` (web-vitals pkg, LCP/INP/CLS/FCP/TTFB) → public `POST /api/analytics/web-vitals` (rateLimit 120/min, zod `.strip()` + per-metric plausibility caps). Server stamps `source='rum_field'` — never client-settable (forged `source`/`rating` keys stripped, regression-tested). Thresholds LCP 2500/4000ms, INP 200/500ms, CLS 0.1/0.25.
- **Organic revenue**: joins 72.4 organic/AI search landings to 72.3 revenue events by session id (lower bound; methodology in `attributionNote`). Composed payload `GET /api/dashboard/search-intelligence` = one Promise.all over 11 storage reads (all date-bounded + LIMITed). Legacy `GET /api/dashboard/analytics/search` kept for backward compat (parity-tested).
- **Drizzle gotcha (fixed)**: JS array param in raw sql template → `ANY(${arr})` fails at runtime with 42809 "op ANY/ALL (array) requires array on right side". Use `IN (${sql.join(items.map((i) => sql`${i}`), sql`, `)})`.
- **Prod deploy step: run `npx tsx scripts/create_phase72_4_tables.ts` against Render DATABASE_URL (idempotent — same script now also creates the 10 new 72.4R tables), then set the 2 GSC env vars when ready.** Tests: `npx tsx scripts/phase72_4_api_tests.ts` (107 checks = 50 original 72.4 + 57 revision: CWV ingestion/forgery/caps, composed auth+shape+first-party parity, GSC fixture import exact aggregation + bucket classification + idempotency, audit findings, recommendations vocab, organic revenue join, SPA-fallback guard; needs running server + `DASHBOARD_PIN`; NOTE: /sync budget is 6 per 5 min → wait 5 min between suite runs or restart the server). check+build PASS. Architect round: 1 high + 2 medium, all fixed, final verdict PASS (no remaining critical/high): (1) sync single-run guard was check-then-create → now DB-enforced via partial unique index `gsc_sync_runs_single_running_uq` ON gsc_sync_runs(status) WHERE status='running' (in schema.ts + bootstrap script; createGscSyncRun sweeps stale >10min running rows, catches 23505 → `GscSyncConflictError` → 409); (2) organic session/revenue joins were unbounded all-time scans → landing-page org CTE bounded to 2×window, organic-revenue queries bounded to 90 days (disclosed in attributionNote); (3) GSC pagination cap (5×5000 rows/set) could truncate silently → pullDimensionSet returns `truncated`, capped sets recorded in setErrors → run status `partial`, never false `success`. Accepted limitation: no HTTP-level 409 concurrency test (needs flaky parallel timing + /sync rate budget is fully consumed by the suite) — the DB unique index is the enforced guarantee.
