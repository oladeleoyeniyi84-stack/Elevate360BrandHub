...
- UI: color-coded `StatusBadge`/`STATUS_META`; CampaignsView 5 summary cards (clickable filters) + filter pills (All+6) + sort (Newest/Oldest/Highest Score/Recently Updated) + per-card quick actions (Open/Approve/Publish/Duplicate/Archive/Delete) + progress bar + score pill. CampaignDetail Mission Control panel (Score/AI Recs/Missing Assets/Suggested Next Action). Generate Everything auto-advances draft→generating→ready_for_review (only when zero failures; "generating" awaited to avoid race). No new env vars.
